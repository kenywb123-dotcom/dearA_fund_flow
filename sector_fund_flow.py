...
SECTOR_LIMIT = 100
REQUEST_DELAY = 0.3
WHITELIST_FILE = os.path.join(CUR_DIR, "概念板块白名单.txt")
BLACKLIST_FILE = os.path.join(CUR_DIR, "概念板块黑名单.txt")
NUMBER = 5         # ???????N???
...
def plot_chart(top10_in, top10_out, forced_sectors, all_data, output):
    ...
    ax.set_title(f"A股概念板块主力资金流向 TOP{NUMBER}\n{ns}  |  数据来源: 东方财富",
...
def main():
    ...
    if not inflow_sectors:
        ...
        sys.exit(1)
    print(f"  获取到 {len(inflow_sectors)} 个概念板块")
    # 导出全部板块名称到txt（供黑白名单参考）
    all_names_path = os.path.join(CUR_DIR, "all_sectors.txt")
    ...
    # ========== [3/3] 绘图 ==========
    print("\n[3/3] 绘制图表...")
    out = os.path.join(CUR_DIR, f"概念板块_{now.strftime('%Y-%m-%d')}.png")
    if ok > 0:
